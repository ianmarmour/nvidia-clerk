...
func main() {
	...
	list := flag.Bool("list", false, "Allows you to list what models are supported in your region.")
	flag.Parse()

	config, configErr := config.Get(region, model, delay, *twilio, *discord, *twitter, *telegram, *desktop, *list)
	if configErr != nil {
		log.Fatal(configErr)
	}
